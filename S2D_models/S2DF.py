...
class S2DF(nn.Module):
    ...
    def forward(self, x):
        # x.shape (1, 3, h(512), w(704))  range[0, 1]
        y = []

        y.append(x) #raw feature
        x = self.block1(x)  # (1, 64, 512, 704)
        if (self.num_block > 1 and self.dense) or self.num_block == 1:
            y.append(x)

        x = self.block2(x) if self.num_block>=2 else x   # (1, 64, 512, 704)
        if (self.num_block > 2 and self.dense) or self.num_block == 2:
            y.append(x)

        x = self.block3(x) if self.num_block>=3 else x   # (1, 64, 512, 704)
        ...
        # torch.cat(y,dim=1).shape: torch.Size([1, 195(3+64+64+64), 512, 704])

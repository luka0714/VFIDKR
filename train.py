...
def train():
    ...
    if not args.SAVED_MODEL==None:
        ...
        # args.SAVED_MODEL ='./model_weights/our_best.pth'
    ...
    for t in range(args.numEpoch):
        ...
        for i, (X0_half,X1_half, y_half) in enumerate(train_loader):
            ...
            # X0_half.shape: (1, 3, 256, 448)    X1_half.shape: (1, 3, 256, 448)   y_half.shape: (1, 3, 256, 448) 
            # len(train_set): 64611
            ...
            X0 = Variable(X0_half, requires_grad= False)    # torch.size([1, 3, 256, 448])
            X1 = Variable(X1_half, requires_grad= False)    # torch.size([1, 3, 256, 448])
            y  = Variable(y_half,requires_grad= False)     # torch.size([1, 3, 256, 448])

            # len(diffs): 2,    len(offsets): 1,    len(filters): 1,   len(filters): 0
            diffs, offsets,filters,occlusions = model(torch.stack((X0,y,X1),dim = 0))

            # len(pixel_loss): 2,    len(offset_loss): 1,     len(sym_loss): 1  
            # pixel_loss[0]: 未增强的网络输出图像与ground truth的loss     pixel_loss[1]: 增强过后的网络输出图像与ground truth的loss
            # offset_loss感觉是flow与参考图像之间的关系    sym_loss感觉是两个flow之间的关系
            pixel_loss, offset_loss, sym_loss = part_loss(diffs,offsets,occlusions, [X0,X1],epsilon=args.epsilon)

            # args.alpha default:[0, 1] 表示未增强的loss和增强后的loss的占比， default情况下只看增强后的loss
            total_loss = sum(x*y if x > 0 else 0 for x,y in zip(args.alpha, pixel_loss))
            # total_loss += 0.01 * g_occlusion

            training_losses.update(total_loss.item(), args.batch_size)
            if i % max(1, int(int(len(train_set) / args.batch_size )/50.0)) == 0:
                ...
                # print("time :" + str(end_time - sing_time))
        ...
        writer.add_scalar('Train/Loss', round(training_losses.avg, 5), t)
        ...
        print("*****************process one epoch time \t " + str(end-proc_end)+"s ******************")
        ...
        for i, (X0,X1,y) in enumerate(val_loader):
            ...
            with torch.no_grad():
                ...
                # val_total_loss += 0.01 * g_occlusion
        ...
        writer.add_scalar('Test/Loss', round(float(val_total_losses.avg), 5), t)
        writer.add_scalar('Test/PSNR', round(float(val_total_PSNR_loss.avg), 5), t)

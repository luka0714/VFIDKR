...
class DepthFlowProjectionLayer(Function):
    ...
    def forward(ctx, input1, input2, requires_grad):
        ...
        if input1.is_cuda:
            ...
            # count = count.cuda()
            # print("correct")
            count = torch.cuda.FloatTensor().resize_(input1.size(0), 1, input1.size(2), input1.size(3)).zero_()   # (1, 1, 512, 704)
            output = torch.cuda.FloatTensor().resize_(input1.size()).zero_()   # (1, 2, 512, 704)
